{*******************************************************************************
   ...
   Modifiziert am 12.01.2003 Jan Pokrandt Jan@JP-Soft.de

   neue Propertys :
    - Color2 : Farbe für ungerade Zeilen
    - Font2  : Font für ungerade Zeilen


   Änderungen sinf mit JP gekennzeichnet

***************************************************************************}

//  15.10.09-ud.
//    nur noch lauffähig ab Delphi Version 5
...
{JP}fColor2 : TColor; // Farbe für die ungeraden Zeilen
{JP}fFont2  : TFont;  // Font für die ungeraden Zeilen
...
{JP}procedure SetFont2(Value: TFont); virtual;
...
{JP}property Color2 : TColor read FColor2 write FColor2 default clGray;
{JP}property Font2  : TFont read fFont2 write SetFont2;
...
procedure Register;
begin
  RegisterComponents( 'Open-Faktura', [TSortGrid] );  //15.10.09-ud.
end;
...
begin
  ...
  end else //Reverse polarity if descending sort.
...
procedure TSortgrid.SetFont2 (Value: TFont);
begin
  if value <> nil then FFont2.Assign(Value);
  Invalidate;
end;
...
{JP}  if (ACol >= FixedCols) and
{JP}     (ARow >= FixedRows) and
{JP}     (arow and 1 = 1) and
{JP}     ( (acol<>col) or (arow<>row) ) then
{JP}  begin
{JP}    canvas.brush.color :=fColor2;
{JP}    Canvas.Rectangle (ARect.left,arect.top,arect.right,arect.bottom);
{JP}    Canvas.Font.Assign (fFont2);
{JP}  end;
   ...
   begin
      ...
      begin
         if FCaseSensitive then
            ...
            pStr := UpperCase( Cells[c, r] );
			if AnsiPos( GFindString, pStr ) > 0 then
         begin
				aCol := c;
				aRow := r;
            Result := True;
