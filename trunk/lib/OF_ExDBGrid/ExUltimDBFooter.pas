{
ExDBGrid Component Suite Version 3.9 - ExUltimDBFooter
...
type
   ...
   public
      constructor Create(Footer: TExDBFooter);
   end;
   ...
   public
      constructor Create(FooterBar: TExDBFooter);
   ...
   TDrawPanelEvent = procedure(FooterBar: TExDBFooter; Panel: TFooterPanel; const Rect: TRect)
      of object;
   TBeforeDisplayTextEvent = procedure(FooterBar: TExDBFooter; Column: TFooterColumn;
      const Value: Variant; var Text: string; var Style: TFooterPanelStyle) of object;
   TBeforeReadFieldsEvent = procedure(FooterBar: TExDBFooter) of object;
   TReadFieldValueEvent = procedure(FooterBar: TExDBFooter; Column: TFooterColumn;
      const Value: Variant) of object;

   TExDBFooter = class(TWinControl)
   private
      ...
      procedure SetExDBGrid(Value: TExDBGrid);
   protected
   ...
   published
      ...
      property ExDBGrid: TExDBGrid read FExDBGrid write SetExDBGrid default nil;
      property DragCursor;
      property DragKind;
      property DragMode;
      property Enabled;
...
constructor TUltimDataLink.Create(Footer: TExDBFooter);
begin
   inherited Create;
   FFooterCtrl := Footer;
   FRecCount := 0;
...
constructor TFooterPanels.Create(FooterBar: TExDBFooter);
begin
...
constructor TFooterColumns.Create(FooterBar: TExDBFooter);
begin
...
constructor TExDBFooter.Create(AOwner: TComponent);
begin
...
destructor TExDBFooter.Destroy;
begin
   if FExDBGrid <> nil then
      begin
         FExDBGrid.UnregisterFooter(Self);
         FExDBGrid := nil;
      end;
...
procedure TExDBFooter.CreateParams(var Params: TCreateParams);
const
...
procedure TExDBFooter.CreateWnd;
begin
   inherited CreateWnd;
...
procedure TExDBFooter.DrawPanel(Panel: TFooterPanel; const Rect: TRect);
begin
...
procedure TExDBFooter.SetPanels(Value: TFooterPanels);
begin
   FPanels.Assign(Value);
end;

procedure TExDBFooter.DoRightToLeftAlignment(var Str: string;
...
procedure TExDBFooter.CMBiDiModeChanged(var Message: TMessage);
var
   Loop: Integer;
begin
   inherited;
...
procedure TExDBFooter.FlipChildren(AllLevels: Boolean);
var
...
procedure TExDBFooter.SetSizeGrip(Value: Boolean);
begin
...
procedure TExDBFooter.SyncToSystemFont;
begin
...
procedure TExDBFooter.UpdatePanel(Index: Integer; Repaint: Boolean);
var
   Flags: Integer;
   S: string;
   PanelRect: TRect;
...
procedure TExDBFooter.UpdatePanels(UpdateRects, UpdateText: Boolean);
const
   MaxPanelCount = 128;
var
...
procedure TExDBFooter.CMWinIniChange(var Message: TMessage);
begin
   inherited;
...
procedure TExDBFooter.CNDrawItem(var Message: TWMDrawItem);
var
   SaveIndex: Integer;
begin
...
procedure TExDBFooter.WMPaint(var Message: TWMPaint);
begin
   UpdatePanels(False, True);
   inherited;
end;

procedure TExDBFooter.WMSize(var Message: TWMSize);
begin
...
function TExDBFooter.IsFontStored: Boolean;
begin
   Result := not FUseSystemFont and not ParentFont and not DesktopFont;
end;

procedure TExDBFooter.SetUseSystemFont(const Value: Boolean);
begin
...
procedure TExDBFooter.CMColorChanged(var Message: TMessage);
begin
   inherited;
   RecreateWnd;
end;

procedure TExDBFooter.CMParentFontChanged(var Message: TMessage);
begin
   inherited;
...
function TExDBFooter.ExecuteAction(Action: TBasicAction): Boolean;
begin
   Result := inherited ExecuteAction(Action);
end;

procedure TExDBFooter.CMSysColorChange(var Message: TMessage);
begin
   inherited;
   RecreateWnd;
end;

procedure TExDBFooter.CMSysFontChanged(var Message: TMessage);
begin
   inherited;
   SyncToSystemFont;
end;

procedure TExDBFooter.ChangeScale(M, D: Integer);
begin
...
procedure TExDBFooter.SetColumns(Value: TFooterColumns);
begin
   FColumns.Assign(Value);
end;

procedure TExDBFooter.Notification(AComponent: TComponent; Operation: TOperation);
begin
   inherited Notification(AComponent, Operation);
   if (Operation = opRemove) and (AComponent = FExDBGrid) then
      begin
         FExDBGrid := nil;
         FPanels.Clear;
      end;
...
procedure TExDBFooter.SetExDBGrid(Value: TExDBGrid);
begin
   if FExDBGrid <> Value then
      begin
         if Assigned(FExDBGrid) then
            FExDBGrid.UnregisterFooter(Self);
         FExDBGrid := Value;
         FPanels.Clear;
         if Assigned(FExDBGrid) then
            FExDBGrid.RegisterFooter(Self);
      end;
   if Value <> nil then
      Value.FreeNotification(Self);
end;

procedure TExDBFooter.FooterMessage(Msg: Integer);
begin
   case Msg of
      FTR_ACTIVE:
         begin
            FDataLink.DataSource := FExDBGrid.DataSource;
      ...
      FTR_SCROLL:
         begin
            if FLeftPanel <> FExDBGrid.LeftCol then
               UpdateDataPanels(False);
         end;
   end;
end;

procedure TExDBFooter.CalcFields;
var
   RecCount: Integer;
   FtrCol: Integer;
   Field: TField;
...
begin
   ...
   try
      ...
      FExDBGrid.RestoreGridPosition;
      FDataLink.DataSet.EnableControls;
   end;
end;

procedure TExDBFooter.UpdateDataPanels(ReCalc: Boolean);
...
var
   ...
   begin
      if FExDBGrid.Columns[GridCol].Visible then
         with FPanels.Add do
            begin
               Trouve := False;
               for FtrCol := 0 to FColumns.Count - 1 do
                  if CompareText(FExDBGrid.Columns[GridCol].FieldName,
                     ...
                     begin
                        ...
                        if IgnoreResizing then
                           ...
                           Width := FExDBGrid.Columns[GridCol].Width;
                        Trouve := True;
                        Break;
                     end;
               if not Trouve then
               ...
               if dgColLines in FExDBGrid.Options then
                  Width := Width + TDrawGrid(FExDBGrid).GridLineWidth;
            end;
   end;
...
begin
   ...
   try
      FPanels.Clear;
      if (FColumns.Count > 0) and (FExDBGrid <> nil) and FDataLink.Active then
         begin
            if ReCalc then
               CalcFields;
            if dgIndicator in FExDBGrid.Options then
               with FPanels.Add do
                  begin
                     Bevel := pbNone;
                     FieldName := '';
            ...
            {for GridCol := 0 to FExDBGrid.FixedCols - 1 do
                CreatePanels;}
            if IgnoreScrolling then
               begin
                  {FLeftPanel := FExDBGrid.FixedCols + 1;
                  MaxCol := FExDBGrid.Columns.Count - 1;}

                  FLeftPanel := FExDBGrid.FixedCols;
                  MaxCol := FExDBGrid.Columns.Count;
               end
            else
               begin
                  FLeftPanel := FExDBGrid.LeftCol;
                  MaxCol := FLeftPanel + FExDBGrid.VisibleColCount - 1;
                  if MaxCol >= FExDBGrid.Columns.Count then
                     MaxCol := FExDBGrid.Columns.Count - 1;
               end;
