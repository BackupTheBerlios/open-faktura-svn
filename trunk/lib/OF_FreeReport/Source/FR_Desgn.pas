...
//------------------------------------------------------------------------------
//  Änderung:
//    26.02.08-ud. - OnSave/OnSaveAs Event eingebaut.
//------------------------------------------------------------------------------
...
type
  TSaveReportEvent = procedure(Report: TfrReport; var RepName: string; var Saved: boolean) of object; //26.02.08-ud.

  TfrDesignerForm = class;
  TfrDesigner = class(TComponent)  // fake component
  private
    FTemplDir: String;
    FOnSave: TSaveReportEvent;  //26.02.08-ud.
    FOnSaveAs: TSaveReportEvent;  //26.02.08-ud.
  public
  ...
  published
    ...
    //
    property OnSave: TSaveReportEvent read FOnSave write FOnSave;  //26.02.08-ud.
    property OnSaveAs: TSaveReportEvent read FOnSaveAs write FOnSaveAs;  //26.02.08-ud.
  end;
...
begin
  ...
  //26.02.08-ud. -BEGIN-
  if (DesignerSelf <> nil) and Assigned(DesignerSelf.OnSaveAs) then
  begin
    b := True;
    s := CurDocName;
    DesignerSelf.OnSaveAs(CurReport, s, b);
    if b then
    begin
      CurDocName := s;
      WasOk := True;
...
begin
  ...
  begin
    ...
    if (DesignerSelf <> nil) and Assigned(DesignerSelf.OnSave) then
    begin
      b := True;
      s := CurDocName;
      DesignerSelf.OnSave(CurReport, s, b);
      if b then
        Modified := False;
    end else
    //26.02.08-ud. -ENDE-
