...
//------------------------------------------------------------------------------
//  Änderung:
//    26.02.06-ud.  nur noch nach Komponenten auf den Owner-Form zulassen.
//------------------------------------------------------------------------------
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        //01.11.09-ud. -BEGIN-
        if CurReport.SearchDatasetType = sdtStandard then
        begin
          // bei Standard alles durchsuchen...
          if f = Owner then
            List.Add(c.Name)
          else
            List.Add(f.Name + '.' + c.Name);
        end else
        if CurReport.SearchDatasetType = sdtOnlyOwnerForm then
        begin
          // nur das eigene Form durchsuchen...
          if f = Owner then
            List.Add(c.Name);
        end else
        if CurReport.SearchDatasetType = sdtNotSearch then
        begin
          // es müssen trotzdem die TfrDataset Komponenten gesucht werden.
          // aber nur im eigenen Form.
          if (f = Owner) and (c is TfrDataset) then
            List.Add(c.Name);
        end;
        //01.11.09-ud. -ENDE-
        ...
        //26.02.06-ud. -BEGIN-
        (*
        // nur noch nach Komponenten auf den Owner-Form zulassen.
        {
        if f = Owner then
          List.Add(c.Name)
        else
  ...
  { TODO : hier table liste einbauen }
begin
  //01.11.09-ud. -BEGIN-
  if CurReport.SearchDatasetType = sdtNotSearch then
  begin
    List.Clear;
    if ClassRef = TDataset then
    begin
      // wir nach TDataset gesucht, dann Liste übergeben.
      List.AddStrings(CurReport.DatasetStrList);
    end;
    // nach TfrDataset, im eigenen Form, suchen...
