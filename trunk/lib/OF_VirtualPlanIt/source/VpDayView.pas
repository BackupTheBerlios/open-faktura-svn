...
NH 22.12.2004 JclDateTime aufgenommen für FormatDateTime inkl. Kalenderwoche (w)
NH 22.12.2004 DateUtils aufgenommen für DaysInAMonth Funktion
...
uses
  ...
  VpCanvasUtils, Menus, JclDateTime, DateUtils;
...
var
  ...
  begin
    ...
    try
      ...
      { Fix Date String }
      {NLH 22.12.2004 in FDateLabelFormat ist (w) für Kalenderwoche möglich}
      DateStr := jclDateTime.FormatDateTime(FDateLabelFormat, RenderDate);
  ...
  var
    ADEventsList       : array of TList; //JAN
    TempList           : TList;
    I, J, K, L, w      : Integer;
    Event              : TVpEvent;
    ADEventRect        : TRect;
  ...
  begin
    ...
    try
      ...
      try
        L :=0;
        for I := 0 to pred(RealNumDays) do
        begin
          { skip weekends }
          ...
          inc(L); SetLength(ADEventsList,L);  //JAN
          ADEventsList[L-1] := TList.Create;  //JAN
          ...
          begin
            if AdEventsList[L-1].Count > 0 then
            begin
              for K := 0 to pred(ADEventsList[L-1].Count) do
              begin
                if TVpEvent(AdEventsList[L-1][K]) = TVpEvent(TempList[J]) then
                begin
                  Skip := true;
                  Break;
                end;
              ...
              if not Skip then
                AdEventsList[L-1].Add(TempList[J]);
            end else
              AdEventsList[L-1].Add(TempList[J]);
          end;
      ...
      begin
        ...
        AdEventRect.Right := AdEventsRect.Right;//RealRight; //JAN
        W  :=(AdEventRect.Right - AdEventsRect.Left) div length(ADEventsList) +1; //JAN

        For L :=0 to Length(ADEventsList)-1 do
        begin
          AdEventRect.Left := AdEventsRect.Left + {(TextMargin div 2)} + L*W + GutterWidth div 2;

          AdEventRect.Right := AdEventRect.Left + W - 1; //JAN


          for I := 0 to pred(ADEventsList[L].Count) do
          begin
            Event := ADEventsList[L][I];
      ...
      if length(ADEventsList)>0 then
      begin
        for i:=0 to length(AdEventsList)-1 do ADEventsList[i].Free;
        setlength(AdEventsList,0);
      end;
    end;
  end;
