...
{* Contributor(s):                                                            *}
{*                                                                            *}
{* 03.01.2005 Jan Pokrandt (Jan@JP-Soft.de)                                   *}
{* - Added property and code for displaying the german "Kalenderwoche"        *}
{*   in the vpmonthview an first left col                                     *}
{*   This needs the JediVCL for the FormatDateTime-Routines                   *}
{*                                                                            *}
{* ***** END LICENSE BLOCK *****                                              *}
{******************************************************************************}
...
type
  ...
  protected{ private }
    FShowKW            : Boolean; // Wenn True, werden die Kalenderwochen Links
                                  // als 1. Spalte angezeigt
    FKBNavigate        : Boolean;
    ...
    procedure SetShowKW(Value:Boolean);
    { internal methods }
    procedure mvHookUp;
  ...
  published
    ...
    //JAN
    property ShowKalenderWochen : Boolean
      read FShowKW write SetShowKW default True;
    //JAN ENDE
...
uses
  SysUtils, Math, Forms, Dialogs, VpEvntEditDlg, JclDateTime;
...
var
  ...
  begin
    ...
    begin
       KWColWidth :=30;
       mvColWidth := (RealWidth - 4 - KWColWidth) div 7; //JAN 7;
    end
       else
    begin
       KWColWidth :=0;
  ...
  begin
    ...
    if HeadStrLen > HeadTextRect.Right - HeadTextRect.Left then
    begin
  ...
  begin
    ...
    if FShowKW then
    begin
       DayTag := Ord(WeekStartsOn)-1;
       J :=-1;
       dhRect.Right := dhRect.Left + KWColWidth;
    end
       else
    begin
  ...
  begin
    ...
    if FShowKW then
    begin
       TextRect.BottomRight := Point (TextRect.Left +  KWColWidth,
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                Drawn  := false;
                if Assigned(FOwnerDrawCells) then
                begin
              ...
              begin
                ...
                begin
                  ...
                  if DayNumber<0
                   then DT :=trunc(StartingDate)
                   else DT :=trunc(StartingDate + DayNumber);

                  Str :=JclDateTime.FormatDateTime('W.',DT);
...
begin
  ...
  if (D > VpMisc.DaysInMonth(Y, M)) then
    D := VpMisc.DaysInMonth(Y, M);
...
procedure TVpMonthView.SetShowKW(Value:Boolean);
begin
     if Value <> FShowKW then
     begin
        FShowKW :=Value;
        Invalidate;
     end;
...
begin
  if FKBNavigate then
    case Key of
      ...
      VK_END   : begin
        DecodeDate(Date, Y, M, D);
        if D = VpMisc.DaysInMonth(Y, M) then begin
          if M = 12 then begin
            M := 1;
            Inc(Y);
          end else
        ...
        Date := EncodeDate(Y, M, VpMisc.DaysInMonth(Y, M));
      end;
      VK_TAB   :
